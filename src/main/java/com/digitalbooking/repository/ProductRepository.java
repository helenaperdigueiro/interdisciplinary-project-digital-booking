...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query("""
            select p from Product p inner join p.reservations r on p.city.name = :name
            and (:start  between r.startDate and r.endDate or :end  between r.startDate and r.endDate or r.startDate between :start and :end)
            """)
    List<Product> findByCityDateAvailable(String name, Date start, Date end);

    @Query("""
            select p from Product p inner join p.reservations r
            on (:start  between r.startDate and r.endDate or :end  between r.startDate and r.endDate or r.startDate between :start and :end)
            """)
